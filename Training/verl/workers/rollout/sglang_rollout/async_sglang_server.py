...
class SGLangHttpServer:
    ...
    def __init__(
        ...
        self._base_http_port = 30000  # Base port for SGLang HTTP servers
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        self._server_port, self._server_task = await run_unvicorn(app, server_args, self._server_address, fixed_port=fixed_http_port)
    ...
    async def generate(
        ...
        finish_reason_info = output.get("meta_info", {}).get("finish_reason")
        finish_reason = finish_reason_info.get("type") if isinstance(finish_reason_info, dict) else None
        
        if return_logprob:
            output_token_logprobs = output["meta_info"]["output_token_logprobs"]
            log_probs, token_ids = zip(
                *[(log_prob, token_id) for log_prob, token_id, _ in output_token_logprobs]
            )
        else:
...
class SGLangReplica(RolloutReplica):
    ...
    async def launch_servers(self):
        ...
        # launch http server in each node 启动HTTP服务器
