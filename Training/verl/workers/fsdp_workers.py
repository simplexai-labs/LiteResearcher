...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _build_model(self, config):
        ...
        fsdp_mesh = self.device_mesh
        use_zero2 = self.config.model.fsdp_config.get("use_zero2", False) if hasattr(self.config.model, 'fsdp_config') else False
        sharding_strategy = get_sharding_strategy(fsdp_mesh, use_zero2=use_zero2)
        ...
        if config.strategy == "fsdp":
            reward_module = FSDP(
                ...
                auto_wrap_policy=auto_wrap_policy,
                device_id=get_device_id(),
                sharding_strategy=sharding_strategy,  # zero3 or zero2
                sync_module_states=True,
