...
# AutoModelForImageTextToText. Provide a compatibility shim.
try:
...
def load_valuehead_model(local_path, torch_dtype, model_config, trust_remote_code):
    ...
    try:
        from transformers import AutoModelForVision2Seq as _AutoModelForVisionSeq
    except ImportError:  # transformers >= 5.0
        from transformers import AutoModelForImageTextToText as _AutoModelForVisionSeq
    ...
    if type(model_config) in _AutoModelForVisionSeq._model_mapping.keys():
        module_class = _AutoModelForVisionSeq
    else:
