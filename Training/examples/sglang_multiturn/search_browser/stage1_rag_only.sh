#!/bin/bash
# =====================================================================
#  LiteResearcher  Stage-1 RL Training
# ---------------------------------------------------------------------
#  Single-node 8xH20.  Runs GRPO + TIS on top of the Qwen3-4B SFT
#  cold-start, training the model to use a local-RAG Google-Search tool
#  and a Browse tool through the SGLang multi-turn agent loop.
#
#  Prerequisites (start BEFORE launching this script):
#    1. Google Search service                         (RAG diskANN server)
#         examples/sglang_multiturn/search_browser/rag_diskann/start.sh
#    2. Browse service                                (browser_service.py)
#         examples/sglang_multiturn/search_browser/tool_backend/start_browse.sh
#    3. LLM Judge endpoint   (any OpenAI-compatible chat completions)
#         configured via tool_backend/.env  ($LLM_JUDGE_API_BASE etc.)
#
#  All paths default to the host LiteResearcher was authored on.
#  Override by exporting any of the following before invoking:
#    PROJECT_DIR   CONDA_ENV   TRAIN_DATA   VAL_DATA   MODEL_PATH
#    CHECKPOINT_PATH   (only used when RESUME_MODE="resume_path")
# =====================================================================
...
# To resume from a previous run, set RESUME_MODE="resume_path" and provide CHECKPOINT_PATH.
...
EXPERIMENT_NAME="local_rag_only_mask_token-mean-seq-mean-temp_0.7_length_32k_nokl"
...
python3 -m verl.trainer.main_ppo \
    ...
    data.shuffle=True \
    +data.seed=42 \
    actor_rollout_ref.model.path="${MODEL_PATH:-/share/project/wanli/Search_Agent/LLaMA-Factory/saves/qwen3-4b-sft-cold-start}" \
    ...
    +actor_rollout_ref.rollout.multi_turn.max_turn_length=2048 \
