#!/usr/bin/env python3
"""合并 rag_direct_wiki 数据为 stage2_wiki.parquet"""
import json
import pandas as pd
import numpy as np
...
OUTPUT = "/share/project/wanli/Search_Agent/verl/data/deepresearch_rl/only_local_rag_stage2/stage2_wiki.parquet"
...
# 加载所有 rag_direct_wiki 文件（排除 rag_direct_wiki_0）
files = [
    f"rag_direct_wiki_{i}.parquet" for i in range(1, 8)
]
...
# 验证 mask_url 和 tools_kwargs
mask_url_count = df_check['extra_info'].apply(lambda x: bool(x.get('mask_url', ''))).sum()
tools_kwargs_count = df_check['extra_info'].apply(lambda x: 'tools_kwargs' in x).sum()
print(f"\n✅ mask_url 有效数量: {mask_url_count}/{len(df_check)}")
print(f"✅ tools_kwargs 存在数量: {tools_kwargs_count}/{len(df_check)}")
