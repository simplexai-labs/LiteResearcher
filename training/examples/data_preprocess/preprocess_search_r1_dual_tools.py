...
# MODIFIED: Made the instruction generic to support multiple tools.
...
def process_single_row(row, current_split_name, row_index):
    ...
    # MODIFIED: Build shared create_kwargs and then tools_kwargs for both search and browse
    # Create a shared dictionary for tool arguments
    ...
    # Build tools kwargs structure for both tools
    tools_kwargs = {
    ...
    with tempfile.TemporaryDirectory() as tmp_download_dir:
        for split in ["train", "test"]:
            ...
            try:
                ...
                # MODIFIED: Update the output filename to be more descriptive
                output_filename = f"{split}_with_tools.parquet"
                output_file_path = os.path.join(local_save_dir, output_filename)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Download Search-R1 from HuggingFace, process for multi-tool use, and save to Parquet.")
    parser.add_argument("--hf_repo_id", default="PeterJinGo/nq_hotpotqa_train", help="HuggingFace dataset repository ID.")
    # MODIFIED: Changed default local_dir to reflect the new data format
    parser.add_argument("--local_dir", default="data/searchR1_processed_with_dual_tools", help="Local directory to save the processed Parquet files.")
